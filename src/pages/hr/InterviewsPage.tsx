  ...
  return <Dialog open={!!entry} onOpenChange={onClose}>
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-2xl">
        ...
        <div className="space-y-4">
          {entry.answers.length === 0 && <p className="py-6 text-center text-sm text-muted-foreground">No questions yet.</p>}
          {entry.answers.map((a, i) => <div key={a.id} className="rounded-lg border p-4">
              <div className="flex items-start justify-between gap-3">
                <div className="text-sm font-medium">Q{i + 1}. {a.question}</div>
                {a.score && <span className={`shrink-0 text-sm font-bold ${scoreColor(a.score * 10)}`}>
                    {a.score}/10
                  </span>}
              </div>
              ...
              {a.answer && <p className="mt-3 whitespace-pre-wrap text-sm text-muted-foreground">{a.answer}</p>}
              {a.feedback && <div className="mt-3 rounded-md bg-primary/5 p-3 text-xs text-muted-foreground">
...
export default function InterviewsPage() {
  ...
  return <div className="space-y-6 text-left">
      ...
      {interviews.error && <p className="rounded-md border border-destructive/30 bg-destructive/10 px-3 py-2 text-sm text-destructive">
          {interviews.error}
        </p>}

      {list.length === 0 ? <Card className="border-dashed">
          ...
          <CardContent className="pt-4">
            <Table>
              ...
              <TableBody>
                {list.map(e => <TableRow key={e.interview.id} className="cursor-pointer" onClick={() => setSelected(e)}>
                    <TableCell>
                    ...
                    <TableCell className="text-right">
                      <StatusBadge status={e.interview.status} label={INTERVIEW_STATUS_LABELS[e.interview.status] ?? e.interview.status} />
                    </TableCell>
                  </TableRow>)}
              </TableBody>
            </Table>
